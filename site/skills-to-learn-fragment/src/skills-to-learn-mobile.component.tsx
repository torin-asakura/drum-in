import React      from 'react'

import { Box }    from '@ui/layout'
import { Column } from '@ui/layout'
import { Layout } from '@ui/layout'
import { Row }    from '@ui/layout'
import { Text }   from '@ui/text'
import { Button } from '@ui/button'

const SkillsToLearnMobile = () => (
  <Row justifyContent='center' display={['flex', 'flex', 'none']}>
    <Column width='100%'>
      <Layout flexBasis={59} />
      <Row>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
        <Box
          borderRadius={50}
          height={['11px', '11px', '18px']}
          width={['11px', '11px', '18px']}
          border='1px solid black'
        />
        <Layout flexBasis={[11, 11, 20]} flexShrink={0} />
        <Box
          width={['280px', '280px', '670px']}
          height={['472px', '472px', '358px']}
          border='1px solid black'
        >
          <Layout>
            <Text>text</Text>
          </Layout>
        </Box>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Box width='324px' height='320px' border='1px solid black' display={['flex', 'flex', 'none']}>
        <Layout>photo</Layout>
      </Box>
      <Layout flexBasis={[10, 10, 39]} />
      <Row justifyContent='center' display={['flex', 'flex', 'none']}>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
        <Layout width='26px' height='6px'>
          <Box width='3px' height='3px' border='1px solid black' borderRadius={50} />
          <Layout flexBasis={10} flexShrink={0} />
          <Box width='3px' height='3px' border='1px solid black' borderRadius={50} />
        </Layout>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Row justifyContent='center'>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
        <Box
          borderRadius={50}
          height={['11px', '11px', '18px']}
          width={['11px', '11px', '18px']}
          border='1px solid black'
        />
        <Layout flexBasis={[11, 11, 20]} flexShrink={0} />
        <Box
          width={['280px', '280px', '670px']}
          height={['472px', '472px', '358px']}
          border='1px solid black'
        >
          <Layout>
            <Text>text</Text>
          </Layout>
        </Box>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Row justifyContent={'center'}>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
        <Box
          borderRadius={50}
          height={['11px', '11px', '18px']}
          width={['11px', '11px', '18px']}
          border='1px solid black'
        />
        <Layout flexBasis={[11, 11, 20]} flexShrink={0} />
        <Box
          width={['280px', '280px', '670px']}
          height={['472px', '472px', '358px']}
          border='1px solid black'
        >
          <Layout>
            <Text>text</Text>
          </Layout>
        </Box>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Box width='324px' height='320px' border='1px solid black' display={['flex', 'flex', 'none']}>
        <Layout>photo</Layout>
      </Box>
      <Layout flexBasis={[10, 10, 39]} />
      <Row justifyContent='center' display={['flex', 'flex', 'none']}>
        <Layout width='26px' height='6px'>
          <Box width='3px' height='3px' border='1px solid black' borderRadius={50} />
          <Layout flexBasis={10} flexShrink={0} />
          <Box width='3px' height='3px' border='1px solid black' borderRadius={50} />
        </Layout>
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Row justifyContent='center'>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
        <Box
          borderRadius={50}
          height={['11px', '11px', '18px']}
          width={['11px', '11px', '18px']}
          border='1px solid black'
        />
        <Layout flexBasis={[11, 11, 20]} flexShrink={0} />
        <Box
          width={['280px', '280px', '670px']}
          height={['472px', '472px', '358px']}
          border='1px solid black'
        >
          <Layout>
            <Text>text</Text>
          </Layout>
        </Box>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Row justifyContent='center'>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
        <Box
          borderRadius={50}
          height={['11px', '11px', '18px']}
          width={['11px', '11px', '18px']}
          border='1px solid black'
        />
        <Layout flexBasis={[11, 11, 20]} flexShrink={0} />
        <Box
          width={['280px', '280px', '670px']}
          height={['472px', '472px', '358px']}
          border='1px solid black'
        >
          <Layout>
            <Text>text</Text>
          </Layout>
        </Box>
        <Layout flexBasis={15} flexShrink={0} display={['flex', 'flex', 'none']} />
      </Row>
      <Layout flexBasis={10} display={['flex', 'flex', 'none']} />
      <Box width='324px' height='320px' border='1px solid black' display={['flex', 'flex', 'none']}>
        <Layout>photo</Layout>
      </Box>
      <Layout flexBasis={30} flexShrink={0} display={['flex', 'flex', 'none']} />
      <Box
        width='324px'
        height='464px'
        display={['flex', 'flex', 'none']}
        border='1px solid black'
        zIndex={2}
      >
        <Layout flexBasis={20} flexShrink={0} />
        <Column width='100%' alignItems='center'>
          <Layout flexBasis={30} flexShrink={0} />
          <Layout>
            <Text>text</Text>
          </Layout>
          <Layout flexBasis={390} />
          <Layout>
            <Button height='41px' width='237px' border='1px solid black'>
              <Layout>
                <Text>ask questions</Text>
              </Layout>
            </Button>
          </Layout>
          <Layout flexBasis={30} flexShrink={0} />
        </Column>
        <Layout flexBasis={20} flexShrink={0} />
      </Box>
    </Column>
    <Layout flexBasis={[15, 15, 52]} flexShrink={0} />
  </Row>
)

export { SkillsToLearnMobile }
