...
const ProgressBar = ({ scrollId }) => {
  const progressRef = useRef<SVGPathElement | null>(null as any)
  ...
  useEffect(() => {
    const renderProgress = (() => {
      ...
      const heightSvg = parseInt(getComputedStyle(svgRef.current!).height, 10)
      const halfHeightWindow = window.innerHeight / 2
      const points = document.querySelectorAll<HTMLElement>('.progress__point')
      return () => {
        points.forEach((el) => {
          const { y } = el.getBoundingClientRect()
          el.style.fill = y - halfHeightWindow <= 0 ? '#f2f2f2' : '#807f7f' // eslint-disable-line no-param-reassign
        })
        ...
        progressRef.current!.style.strokeDashoffset = (dashOffset - offsetUnit).toString()
      }
    })()
    document.addEventListener('scroll', renderProgress)
    return () => document.removeEventListener('scroll', renderProgress)
  }, [])

  return (
    <Box position='absolute' top={0} data-scroll data-scroll-id={scrollId}>
      <svg
        ref={svgRef}
        className='progress'
        width='130'
      ...
      >
        ...
        <g>
          <path
            d='M 65.7 682 L 127.3 682'
            stroke='url(#paint0_linear_1904_16762)'
            strokeWidth='4'
          />
          <defs>
            <linearGradient
        ...
        <g>
          ...
          <path
            ...
            fill='#807f7f'
          />
          <circle className='progress__point' cx='38.5' cy='170' r='8' fill='#807f7f' />
          <circle className='progress__point' fill='#807f7f' cx='69' cy='325.5' r='8' />
          <circle className='progress__point' fill='#807f7f' cx='41' cy='499.9' r='8' />
          <circle className='progress__point' fill='#807f7f' cx='64.5' cy='682.3' r='8' />
          <circle className='progress__point' fill='#807f7f' cx='45' cy='864.2' r='8' />
        </g>
      </svg>
    </Box>
  )
