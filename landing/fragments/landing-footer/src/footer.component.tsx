...
const FooterBlock = forwardRef<HTMLDivElement, FooterProps>(({ footerData,buttonUp = true }, ref: any) => (
        ...
        <Column width='100%'>
          ...
          <Box width='100%' flexDirection={['column', 'column', 'row']}>
            <Box width={[120, 170, 220]} height={[24, 34, 44]} flexShrink={0}>
              <NextLink path='/'>
                <ImageBlock src={footerData?.logo?.node.sourceUrl || ''} />
              </NextLink>
            </Box>
            ...
            <Column>
              ...
              {footerData?.courses?.nodes.map((element, index: number) => {
                if (index === 3) {
                  return (
            ...
            <Column>
              ...
              <Layout flexBasis={[20, 22, 24]} />
              {footerData?.socials?.map((element) => (
                <React.Fragment key={uniqid()}>
